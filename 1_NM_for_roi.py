...
###################################CONFIG #####################################
#path
data_dir = 's_normative_model/s_aca_nm/data/'
root_dir = '/s_normative_model/s_aca_nm'
out_dir = os.path.join(root_dir,'clinical_model') 
blr_dir = os.path.join(root_dir,'clinical_model', 'clinical_blr' ) #

#load covariate+ acapulco output
cov_dir='covariates/'
df_tr=pd.read_csv(cov_dir+'bl_aca_cov_tr.csv')
df_te=pd.read_csv(cov_dir+'bl_aca_cov_te_clinical.csv')
...
roi_ids_df=pd.DataFrame(roi_ids)

cols_cov = ['age','sex']
site_ids =  sorted(set(df_tr['site'].to_list())) 

# load the idps to process: region names
with open(os.path.join(data_dir,'roi_ids.txt')) as f:
...
for nummer, idp in enumerate(idp_ids): 
    ...
    #impute bad subjects with std 5
    ...
    #X_tr=pd.read_csv(os.path.join(out_dir,'cov_bspline_tr.csv'))
    #X_tr=X_tr.to_numpy()
    #X_te=pd.read_csv(os.path.join(out_dir,'cov_bspline_te.csv'))
    #X_te=X_te.to_numpy()
    #cov_file = os.path.join(out_dir, 'cov_bspline')
...
blr_metrics.to_csv(os.path.join(blr_dir,'blr_metrics.csv'))
